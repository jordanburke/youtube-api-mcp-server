...
export class ShortsManager implements MCPFunctionGroup {
  ...
  async findShortSegments({ videoId, maxSegments = 3 }: { videoId: string; maxSegments?: number }): Promise<any[]> {
    try {
  ...
  private identifyInterestingSegments(markers: any[], maxSegments: number): any[] {
    ...
    for (let i = 0; i < markers.length; i++) {
      const segmentMarkers = markers.filter((m) => m.time >= markers[i].time && m.time < markers[i].time + windowSize)
    ...
    return segments.sort((a, b) => b.engagement - a.engagement).slice(0, maxSegments)
  }
  ...
  private determineSegmentType(markers: any[]): string {
    ...
    return Object.entries(typeCount).sort(([, a], [, b]) => (b as number) - (a as number))[0][0]
  }
