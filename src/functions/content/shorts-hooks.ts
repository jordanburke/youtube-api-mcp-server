...
export class ShortsHooksGenerator implements MCPFunctionGroup {
  ...
  async generateHooks({ topic, style = "question" }: { topic: string; style?: string }): Promise<any> {
    try {
  ...
  async analyzeHookPerformance({ videoIds }: { videoIds: string[] }): Promise<any> {
    try {
  ...
  private generatePatternInsights(patterns: any): string[] {
    ...
    const topPhrases = [...patterns.openingPhrases.entries()].sort((a, b) => b[1] - a[1]).slice(0, 3)

    insights.push(`Most effective opening phrase: "${topPhrases[0][0]}"`)
    insights.push(`Top performing format: ${[...patterns.commonFormats.entries()].sort((a, b) => b[1] - a[1])[0][0]}`)
