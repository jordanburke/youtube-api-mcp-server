...
export class ContentAnalysis implements MCPFunctionGroup {
  ...
  async generateSummary({ videoId, maxLength = 250 }: { videoId: string; maxLength?: number }): Promise<string> {
    try {
  ...
  private isSignificantChange(text: string): boolean {
    const indicators = ["next", "now", "let's", "moving on", "first", "second", "finally", "but", "however", "although"]
  ...
  async getRecommendations({ videoId, maxResults = 10 }: { videoId: string; maxResults?: number }): Promise<any[]> {
    try {
